...
Shell、lazygit、tmux 与 AI agent 共存于同一画布。<kbd>⌘</kbd><kbd>F</kbd> 给重要终端加星，再用 <kbd>⌘</kbd><kbd>]</kbd> / <kbd>⌘</kbd><kbd>[</kbd> 在星标范围内循环（用 <kbd>⌘</kbd><kbd>G</kbd> 在「全部终端 / 仅星标 / 整个 worktree」之间切换聚焦层级）。自定义标题、逐 agent CLI 路径覆盖。新建终端会保持你的偏好尺寸——第一次手动调整后即固定为默认，后续「+ Terminal」均沿用，与侧边栏状态无关。
