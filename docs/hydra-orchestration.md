...
1. Start the chosen path:
   ...
   hydra run --task "..." --repo .
   ```
   The default workflow is `planner -> implementer -> evaluator`. For a single direct implementer run, pass `--template single-step`. For a direct isolated worker primitive, use `hydra spawn --task "..." --repo .`.
