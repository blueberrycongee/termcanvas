...
export function renderTaskPackageTemplate(contract: HandoffContract): string {
  const lines = [
    ...
    "- Check `task_status`, `last_meaningful_progress_at`, `last_session_event_at`, `turn_state`, `foreground_tool`, and `active_tool_calls` before deciding takeover/retry.",
    "- Keep waiting when telemetry shows `task_status=running`, recent meaningful progress, fresh session events, `thinking`, `tool_running`, `tool_pending`, active tool calls, or a foreground tool.",
    "- For Codex, do not take over from one `stall_candidate` alone; confirm no fresh session events and `active_tool_calls=0` first.",
