...
export const COMPUTER_USE_STATUS_GUIDANCE = {
  ...
  protocol: [
    ...
    "Use get_cursor_position for coordinate debugging or relative follow-up actions, not as a substitute for fresh get_window_state screenshots.",
    "Use set_recording/get_recording_state/replay_trajectory for JSON trajectory capture and replay when debugging or reproducing Computer Use behavior.",
...
6. Use get_cursor_position for coordinate debugging or relative follow-up actions, not as a substitute for fresh get_window_state screenshots.
7. Use set_recording/get_recording_state/replay_trajectory when debugging or reproducing Computer Use behavior. This records JSON tool trajectories, not video.
8. Call get_window_state or get_app_state before acting. If it is empty or sparse, re-activate and observe again before declaring a limitation.
9. Prefer indexed Accessibility elements and semantic AX actions from get_app_state. For pid-targeted scroll, use direction with by=line/page so the helper can send scroll keystrokes instead of fragile wheel events.
10. For CEF/Chromium/WebGL/media surfaces that remain sparse after one re-observe, use the returned screenshot plus keyboard shortcuts before screenshot-coordinate clicks. For Chromium/Electron text fields, use type_text_chars or type_text with delay_ms around 30 so autocomplete and IME paths keep up.
11. Use move_cursor for hover-revealed UI, tooltip/menu discovery, or drag pre-positioning when AX and keyboard paths are not sufficient.
12. Use the returned screenshot for observation and use screenshot coordinates only as the last resort. Pass capture_id with screenshot-coordinate actions when available.
13. After every action, call get_app_state again and verify the result before reporting success.
`;
