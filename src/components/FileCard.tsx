...
export function FileCard({ card }: Props) {
  ...
  const isSelected = useSelectionStore((state) =>
    state.selectedItems.some(
      (item) => item.type === "card" && item.cardId === card.id,
    ),
  );

  useEffect(() => {
    register(card.id, { x: card.x, y: card.y, w: card.w, h: card.h });
    return () => unregister(card.id);
  }, [card.h, card.id, card.w, card.x, card.y, register, unregister]);
  ...
  const resolved = allResolved[card.id] ?? { x: card.x, y: card.y };
