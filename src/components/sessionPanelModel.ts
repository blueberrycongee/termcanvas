  ...
  for (const project of projects) {
    for (const worktree of project.worktrees) {
      for (const terminal of worktree.terminals) {
        if (!isCanvasTerminal(project.collapsed, worktree.collapsed, terminal)) {
          continue;
        }

        const telemetry = telemetryByTerminalId.get(terminal.id);
        const session = terminal.sessionId
          ? sessionsById.get(terminal.sessionId)
          : undefined;
        const derived = deriveTerminalState(terminal, telemetry, session);
        const title = resolveTerminalTitle(terminal, worktree.name, project.name);
        const locationLabel =
